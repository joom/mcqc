...
instance Pretty CExpr where
    ...
    group $
      "[=]("
        ...
        <+> pretty _lbody
          <> ";"
        <+> "}"
  pretty CExprCall {_cd = CDef {_nm = "return"}, _cparams = [a]} =
    "return" <+> pretty a <> ";"
  pretty CExprCall {_cd = CDef {_nm = "eqb"}, _cparams = [a, b]} =
    pretty a <+> "==" <+> pretty b
  pretty CExprCall {_cd = CDef {_nm = "ltb"}, _cparams = [a, b]} =
    pretty a <+> "<" <+> pretty b
  pretty CExprCall {_cd = CDef {_nm = "leb"}, _cparams = [a, b]} =
    pretty a <+> "<=" <+> pretty b
  pretty CExprCall {_cd = CDef {_nm = "match"}, ..} =
    "match" <> (parens . breakcommatize $ _cparams)
  ...
  pretty CExprProd {..} =
    "std::make_pair" <> parens (pretty _fst <> "," <+> pretty _snd)
  pretty CExprSeq {..} = pretty _left <> ";" <> line <> pretty _right
  pretty CExprStmt {_sd = CDef {_nm = "_", ..}, ..} = pretty _sbody
  pretty CExprStmt {_sd = CDef {..}, ..} =
    pretty _ty <+> pretty _nm <+> "=" <+> pretty _sbody
...
instance Pretty CDecl where
  pretty CDFunc {_fd = CDef {..}, ..} =
    mkTemplateLine (_ty : map gettype _fargs)
      ...
      <+> pretty _nm
        <> "("
        <> (commatize . raiseFuncs nfreevars $ _fargs)
        <> ") {"
        <> line
        <> (tab . pretty $ _fbody)
        <> line
        ...
        <> line
    where
      nfreevars = maximum . map getMaxVaridx $ (_ty : map gettype _fargs)
  ...
  pretty CDStruct {..} =
    mkTemplateLine (map gettype _fields)
      ...
      <+> "{"
        <> line
        <> (tab . vcat . map (\x -> pretty x <> ";")) _fields
        <> line
        <> tab (pretty _sn <> "(" <> (commatize . map pretty) _fields <> ") {")
        <> line
        <> (tab . tab . vcat . map (\x -> "this->" <> toNm x <+> "=" <+> toNm x <> ";")) _fields
        <> line
        <> tab "};"
        <> line
        <> "};"
...
mkTemplateLine argsT
  | not (null (prettytempl nfreevars argsT)) =
      "template<" <> (commatize . L.nub $ prettytempl nfreevars argsT) <> ">" <> line
  | otherwise = mempty
  where
    nfreevars = maximum . map getMaxVaridx $ argsT
