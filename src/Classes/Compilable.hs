...
instance Compilable Declaration CDecl where
  ...
  comp TermDecl {val = ExprLambda {..}, name = "main", ..} =
    ...
    where
      mkbody CExprCall {_cd = CDef {_nm = "return"}, _cparams = [a]} = CExprSeq a retz
  ...
  comp FixDecl {fixlist = [Fix {name = Just _, body = _}]} =
    error "Fixpoint not followed by an ExprLambda is undefined behavior"
  comp FixDecl {fixlist = [Fix {name = Nothing, ..}]} =
    error "Anonymous Fixpoints are undefined behavior"
  comp FixDecl {fixlist = []} =
    error "Empty fixlist for declaration found, undefined behavior"
  comp FixDecl {fixlist = _ : _} =
    error "Fixlist with multiple fixpoints is undefined behavior"
  comp _ = mempty
...
instance Compilable Expr CExpr where
  comp ExprLambda {..} = CExprLambda defs $ comp body
    where
      defs = map mkauto argnames
  comp ExprCase {..} = CExprCall (mkauto "match") $ comp expr : map mkLambda cases
    where
      mkLambda Case {..} = CExprLambda (mkautos pat) $ comp body
  ...
  comp ExprApply {func = ExprFix {funcs = [Fix {name = Just nm, ..}]}, ..} = fixpoint <> call
    where
      fixpoint = CExprStmt (mkauto nm) $ comp body
      call = CExprCall (mkauto nm) $ map comp args
  comp ExprLet {..} = assignment <> comp body
    where
      assignment = CExprStmt (mkauto name) $ comp nameval
