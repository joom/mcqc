...
instance Typeful CDecl where
  ...
  unify ctx t CDFunc {..} =
    CDFunc
      ...
      <*> pure _fargs
      <*> unify ctx t _fbody
  unify ctx t CDStruct {..} =
    CDStruct _sn
      <$> mapM (unify ctx t) _fields
      <*> pure _nfree
  unify ctx t CDSeq {..} =
    CDSeq
      <$> unify ctx t _left
  ...
  addctx ctx CDType {_td = CDef {_ty = CTExpr {_tbase = "std::variant", ..}, ..}}
    ...
    where
      freedom = maximum . map getMaxVaridx $ _tins
  ...
  isUnifiable ctx d =
    (canonicalizeFn . getname $ d) `M.member` ctx
...
instance Typeful CExpr where
  ...
  isUnifiable ctx c@CExprCall {_cd = CDef {..}, ..} =
    (canonicalizeFn _nm `M.member` ctx) && (isUnifiable ctx . gettype $ c)
