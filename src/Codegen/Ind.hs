...
-- | Intermediate representation for inductive datatypes
...
toCTypeAbs abs t@TArrow {..} = (init . flattenType $ t) --> (last . flattenType $ t)
  where
    flattenType TArrow {..} = toCTypeAbs abs left : flattenType right
...
mkctor abs indtype Ind {..} = CDef name ctyp
  where
    ctyp = map transT argtypes --> indtype
...
contract CDInd {..} =
  mconcatMap (mkCtorStruct unaliasedT) _ictors
    <> mkIndAlias _id _ictors
    <> mconcatMap (mkCtorFunc _id) _ictors
    <> mkMatch _id _ictors
  where
    freedom = maximum $ map getMaxVaridx _ictors
...
toCtorT n nm CTFunc {..}
  ...
  | otherwise = CTBase nm
  where
    freedom = maximum $ n : map getMaxVaridx _fins
...
mkCtorFunc CDef {..} CDef {_nm = ctornm, _ty = CTFunc {..}} =
  CDFunc fdptr defs $
    CExprCall
      (CDef "return" (CTPtr _ty))
      [ -- return inside the ctor body
        CExprCall
          (CDef "std::make_shared" _ty)
          [ -- wrap in a shared pointer
            CExprCall fd . map (CExprVar . getname) $ defs -- Call struct constructor in mkCDStruct
          ]
      ]
  where
    fdptr = CDef (T.toLower ctornm) (CTPtr _ty)
    fd = CDef ctornm _ty
    defs = givenm 'a' _fins
